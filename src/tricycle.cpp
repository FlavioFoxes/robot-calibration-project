...
Tricycle::Tricycle(double timestamp,
                   ...
                   Vector3d pose) : _timestamp(timestamp),
...
{
    std::tuple<double, Vector3d, Vector3d> tuple = predict(get_parameters_to_calibrate(), _pose, actual_tick_traction, next_tick_traction, actual_tick_steer);
